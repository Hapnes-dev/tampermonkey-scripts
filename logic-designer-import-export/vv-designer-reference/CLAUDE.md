...
- **HOST QUIRK (live-confirmed on plant 9839, 2026-07-10):** production plants run a
  **newer/minified designer build** where the simulator's post-completion auto-clear
  ...
  a handle **not reachable via `paper.simulation_stopper`** — `clearTimeout(simulation_stopper)`
  does NOT cancel it (it does on the older demo-plant build; the timed stop also fired at ~2–4 s
  there, not 5). Anyone keeping sim results on-canvas must **wrap `paper.simulator_stop` and
  swallow those two reasons** (the Import/Export Live Simulate panel does this while open;
  error-path stops pass through).
