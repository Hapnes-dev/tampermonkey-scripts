...
- **Wires are coloured by the value they carry**: green = TRUE, red = FALSE, blue = a number, grey = never evaluated — you see where the signal dies at a glance. Blocks the run never evaluated get a dark overlay. Moving blocks around repaints the boxes and colours at the new positions (a move is layout, not a logic change). **🎬 Replay** plays the last run back in slow motion: value boxes appear in evaluation order and each block's outgoing wires *animate* into their colour as the signal spreads — **time-behaviour blocks dwell longer**: a `DELAY_VARIABLE` holds the flow proportionally to its configured seconds (1 s ≈ 40 ms of replay, capped), so a 30 s delay visibly takes time on the wire.
