...
- **With *auto re-run* on, the flow is a live view that stays on the canvas.** Panel value changes re-simulate (debounced), and a watcher also re-simulates when the **canvas** changes (block added/moved/reconfigured/rewired — it waits until the edit settles) and repaints the overlay if the host wipes it. The values stay visible until you click **■ Stop** (or turn auto re-run off / close the panel); Stop suppresses the live view until the next change or ▶ Run.
