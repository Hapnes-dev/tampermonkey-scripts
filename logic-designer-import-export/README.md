...
- **FORMULA blocks compute for real.** The designer's own client simulator cannot evaluate formulas (they run server-side in PHP) — it just *prompts you* for every FORMULA's value, which used to turn every formula into a manual panel input silently defaulting to 0. Live Simulate now evaluates the documented grammar locally (`inp0…inpN‑1`, arithmetic, `%`, comparisons incl. `==`, `?:`, `and`/`or`, `min/max/abs/round/floor/ceil/sqrt/pow/sin/cos/tan/exp/log/log10/fmod/pi()/random()`, `time()`/`date('w|W|N|G|H|i|s|j|d|n|m|Y|y|z|U', …)`, and the PHP-stdlib subset production formulas actually use: `intval`, `idate`, `substr`, `strpos` (PHP-style `false` on miss, so `!== false` works), `strtotime`, plus `null` in ternary branches), gathering inputs **by pin** from the wires — so formula chains (mode decoders, pump-exercise timing math, …) simulate exactly like on the server. Measured against a 71-formula fleet corpus, 67 compile locally; only the rest (`gmdate`, `sort`, server-only calendar built-ins) fall back to a manual `FORMULA (manual)` input row.
