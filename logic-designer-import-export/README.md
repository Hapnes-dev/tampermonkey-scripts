...
- **Wires are coloured by the value they carry**: green = TRUE, red = FALSE, blue = a number, grey = never evaluated — you see where the signal dies at a glance. Blocks the run never evaluated get a dark overlay. Moving blocks around repaints the boxes and colours at the new positions (a move is layout, not a logic change). **⧖ Play flow** plays the last run back so you can follow the entire flow: value boxes appear in evaluation order and each block's outgoing wires **visibly fill with colour from source to target** — the grey wire stays visible while a coloured overlay grows along it (stroke-dashoffset), then the wire takes the colour — **time-behaviour blocks fill slowly**: a `DELAY_VARIABLE` holds the flow proportionally to its configured seconds (1 s ≈ 80 ms of replay, capped), so a 30 s delay visibly creeps along the wire. The ending is a **stable frame**: all values (TRUE/FALSE/numbers) and solid colours stay painted with nothing left to flicker (pending repaints are cancelled, fill effects cleaned up). A **status chip** shows what the simulator is doing — *Ready*, *Values on canvas*, *Live — re-runs on change* (pulsing), *Replaying…* (pulsing) — and Run/Play flow are disabled while a replay is in progress (■ Stop cancels it).
