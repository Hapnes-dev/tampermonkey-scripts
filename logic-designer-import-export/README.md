...
- **Wires are coloured by the value they carry**: green = TRUE, red = FALSE, blue = a number, grey = never evaluated — you see where the signal dies at a glance. Blocks the run never evaluated get a dark overlay. **🎬 Replay** plays the last run back in slow motion: value boxes appear in evaluation order and each block's outgoing wires light up as the signal spreads.
