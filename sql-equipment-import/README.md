...
3. Click an **↳ equipment row** to fetch just that one: its single `iw_sys_order_no` row and its own `iw_par_<link>_groups` / `iw_par_<link>_param` / `iw_set_<base>` tables (`CREATE TABLE IF NOT EXISTS` plus all rows), together with the hosting driver's `iw_sys_plant_settings` (owner = driver_type) and `iw_sys_processes` row. Clicking the **driver row** instead fetches the whole driver — every equipment on it. The donor's real unit rows are **never copied** (v8.4): the template ships three generic example units — `P01 / Pos 01 / 0_1` … — carrying the donor's linkage columns (grp_name, driver_type, regulator_type, order_no), exactly like the curated templates, and the form renumbers them as usual.
