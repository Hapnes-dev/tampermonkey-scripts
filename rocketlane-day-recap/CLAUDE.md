...
> Current version: **4.72**. Always bump `@version` + commit + push (Tampermonkey auto-updates).
...
- **4.48** added the 📋 **Day by category** timesheet roll-up (`.catsum` panel above `.results`). New module before
  ...
  log prefix).
- **4.62–4.72** ⤴ **Book day** — one click writes the split into the Rocketlane timesheet via Rocketlane's own API.
