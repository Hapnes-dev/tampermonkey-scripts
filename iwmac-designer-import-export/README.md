...
Adds a **Panel JSON** section to the IWMAC Designer's manager sidebar (right below *Manage Files*) with four buttons in a compact 2×2 grid — **Export JSON**, **Copy JSON**, **Insert JSON…**, **BG → .ai** (background → Illustrator) — so a panel's complete look (objects, containers, graphics, background image) can be copied out as a single `.json` file and inserted into another panel, on the same plant or a different one, and the background artwork can be handed to Adobe Illustrator for editing. The designer itself has no way to do either; this script adds both.
...
The fourth button (**BG → .ai**, tooltip *Background → Adobe Illustrator*; since v1.3.1 the four buttons sit in a 2×2 grid so the manager sidebar never grows a scrollbar) exports the **current panel's background image as a file Adobe Illustrator edits directly**:
