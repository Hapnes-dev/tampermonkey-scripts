...
Insert JSON also takes **AI-authored** files — generate a panel from a P&ID or system description and insert it, then link the objects by hand. The ready-to-use kit is in [iwmac-designer-reference/](iwmac-designer-reference/): `AI-REQUEST-ROUTING.md` (**read first** — which deliverable a request is actually asking for; the panel-type and output-mode routing that both 2026-08-10 room-control failures skipped), `AI-BRIEFING.txt` (knowledge file for any AI), `AI-AGENT-INSTRUCTIONS.txt` (paste into the M365 Copilot Studio instructions field — 7,914 chars, no `<`/`>`), `DESIGN-OBJECT-CATALOG.md` (**which object do I use** — all 797 palette objects named in plain language with size, tag/link capability, states and real production placement counts, plus a pick-by-task table; upload it as knowledge alongside the briefing), and the example answers in `reference_data/`: `generated-panel-example.json` (CO₂ rack overview from an Advansor ValuePack P&ID, insert-verified 73/73), plus three **real production exports as style ground truth** — `real-vent-panel-example.json` (360.001 Ventilasjon: the normative vent panel, 102 designer objects on the ~6 KB blank background — **Ventilasjon is objects-only, never an AI-drawn SVG background**), `real-vent-panel-example-2.json` (a second plant's 360.001 Ventilasjon on the same blank background: 92 objects / 39 obj_ids, 55 linked, BACNET drivers, horizontal dampers, the sub-page navigation row and a `file_pdf` document link) and `real-spjeldliste-example.json` (360.004 Spjeldliste damper list: the container-built table pattern, 208 rows). The kit also teaches **linking**: hand the agent a plant's `iw_gen_driver_parameters` .sql dump and it fills in `driver_id`/`unit_id` per object (briefing §8b; worked pair: `real-vent-panel-linked-example.json` + `driver-parameters-sample.sql`). And it teaches **drawing the Maskin background** itself: `maskin-drawing-method.txt` is the distilled Advansor CO₂-booster drawing doctrine (layout zones, circuit colors as function — orange HP / yellow receiver / cyan MT suction / blue LT suction — symbol & pill rules, **background colour follows the supplied export or the user's requirement**, Illustrator layer template), with `maskin-light-template.ai` — the production Illustrator template the kit ships (real layers `Backround`/`Rør`/`Maskinrom`, artboard exactly 1400×750 pt) — rendered 1:1 as `maskin-light-style-reference.png`; it is a light template, which describes the committed file rather than mandating a skin — and `generated-maskin-example.json` as the worked answer — a full generated Maskin (method-drawn `image_svg` + 63 objects with the canonical link-map aliases), insert-verified 63/63. Attach the briefing + examples as the agent's knowledge files. For "which plant has a good X to copy" and "how is a panel of type Y built today", the kit also carries a **live fleet survey** (41 Coop Extra plants, 231+ panels, surveyed 2026-08-08): `PLANT-PANEL-CATALOG.md` (per-plant panel inventory), `PANEL-TYPE-GUIDE.md` (per-type style guide with named best copy-sources), and the raw data `reference_data/plant-panel-survey.json`.
