...
[`IWMAC-Designer-Import-Export.user.js`](../IWMAC-Designer-Import-Export.user.js) adds Export JSON / Insert JSON / Background → Illustrator under Manage Files. It deliberately reuses the host verbatim:
...
- **[AI-AGENT-INSTRUCTIONS.txt](AI-AGENT-INSTRUCTIONS.txt)** — the same contract compressed for the M365 Copilot Studio instructions field (7,991 chars, no angle brackets — the field rejects `<`/`>` and caps at 8000).
...
- **Ventilasjon is objects-only.** A vent panel is drawn with duct/pipe/connector/equipment objects on the ~6 KB blank background — **never** an authored `panel.image_svg`. The AI-drawn vent example (`generated-vent-example.json`, 79 objects on an AI SVG background) was deleted on 2026-08-09 so an agent cannot copy that shape; [reference_data/real-vent-panel-example.json](reference_data/real-vent-panel-example.json) is the only vent reference, and all 41 of its obj_ids are real palette entries in [reference_data/all-design-objects.json](reference_data/all-design-objects.json).
