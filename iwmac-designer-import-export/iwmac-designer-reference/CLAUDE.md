...
[`IWMAC-Designer-Import-Export.user.js`](../IWMAC-Designer-Import-Export.user.js) adds Export JSON / Insert JSON / Background → Illustrator under Manage Files, plus an EXPORT XLSX button beside the param popup's UNIT NAME item (§13b) that writes `w2ui.paramgrid.records` as a styled workbook (own td, not a w2ui item — the host row is a radio group). It deliberately reuses the host verbatim:
