...
- **[AI-AGENT-INSTRUCTIONS.txt](AI-AGENT-INSTRUCTIONS.txt)** — the same contract compressed for the M365 Copilot Studio instructions field (7,994 chars, no angle brackets — the field rejects `<`/`>` and caps at 8000).
...
- **[reference_data/generated-vent-example.json](reference_data/generated-vent-example.json)** — a generated Ventilasjon 360.001 with an AI-authored SVG background in `panel.image_svg`, insert-verified live (79/79 objects, 0 errors).
- **[reference_data/generated-maskin-example.json](reference_data/generated-maskin-example.json)** — a generated **Maskin** (CO₂ booster): light-skin `image_svg` drawn per `maskin-drawing-method.txt` (function-coloured pipes, equipment symbols, EMPTY pills) + 63 objects carrying the canonical `maskin-akpc-link-map.json` aliases verbatim (so a later dump-relink resolves them by exact match), insert-verified live (63/63, background applied; the two map ids outside the allowlist — `number_v3_custom_json_obj`, `number_v3_60px_no_conn` — are substituted with `number_v3_value_only` per the NEVER-invent rule).
- **Real production exports as normative style references** (userscript-exported, plant ids masked): [reference_data/real-vent-panel-example.json](reference_data/real-vent-panel-example.json) (a 360.001 Ventilasjon, 102 objects) and [reference_data/real-spjeldliste-example.json](reference_data/real-spjeldliste-example.json) (a 360.004 Spjeldliste damper list — 383 scaffold objects + **208 `objects_container` rows**, the container-built list-panel pattern, briefing §7c).
...
- **Maskin background drawing method**: [reference_data/maskin-drawing-method.txt](reference_data/maskin-drawing-method.txt) — the distilled Advansor CO₂-booster drawing doctrine (from the internal "cooling/freezer machine drawing method v2" Word doc, project identifiers omitted): canvas layout (MT upper-left / LT lower-left / oil-sep on the discharge riser / gas cooler top-right / heat-recovery branch top-center / receiver right-of-center / labeled suction-header exits bottom-right / grey info panel far right), the circuit colour code as **function** (orange = HP/discharge, yellow = receiver/liquid, cyan = MT suction, blue = LT suction) with the Illustrator layer-naming template, symbol/pill/text rules (white pills drawn EMPTY — number objects render into them; dark grey pills = setpoints), the PDF→Illustrator→PNG workflow + QA checklist, and the **light-skin-only** rule — the light skin (`reference_data/maskin-light-style-reference.png`, rendered from `maskin-light-template.ai`) is the **only** sanctioned look; **never draw a dark-background maskin**, not even when the plant's existing maskin is dark. `reference_data/maskin-dark-style-example.png` is kept as historical evidence of what not to produce, never as a drawing reference. Briefing "MASKIN ARTWORK" block carries the compact version.

### Editing an existing Maskin compressor bank from an exported panel JSON

Classify the output before editing. A **background-only** patch changes `image_data`, declares zero counts, and keeps `single_objects`, `containers`, and `graphics` empty. An entire/full JSON preserves the complete source document and exact existing object arrays, appending only explicitly requested objects. **Insert JSON appends objects**, so a full export is safe only on an empty canvas unless duplication is intentional.

Inspect the target column before creating objects; artwork does not automatically require live controls. Treat the nearest role-matching sibling as one atomic compressor-column template and reuse one measured translation vector for compressor art, upper/lower branches, labels, empty pills, and requested object coordinates. In the masked example, fixed-speed C3 supplies status/capacity/runtime for C4; C1's additional VSD row must not be copied. New objects remain unlinked with literal `id`/`driver_id: "driver_id"`, `linked: "false"`, blank link/unit fields, and descriptive aliases. Existing source objects are byte-for-structure preserved unless the request names an authorized change.

Background owns pipes, equipment, static labels, pill/status artwork, and empty fields; dynamic objects own live status/values. Preserve source alpha or mask only intended foreground pixels—never paste an opaque crop. Remeasure each source's centerline, RGBA/alpha, thickness, anti-aliasing, and junction geometry; the fixture's orange `191 / 255 / 64` rows, two opaque cyan rows, and two opaque branch columns are example-specific, not defaults.

Mandatory QA reparses JSON, checks counts/sequential names/exact object diff/protected fields, and renders the full panel, zoomed bank, and background with dynamic overlays. A background-only preview cannot establish overlay alignment; restart from the original export after a failed iteration. See the complete [canonical method](reference_data/maskin-drawing-method.txt) and [masked fixtures](tests/fixtures/maskin-compressor-bank/). Run:
...
```
The userscript also exposes `window.__IWDIE` (`doExport`, `doCopyJson`, `openImportPanel`, `applyImport`, `doExportBackgroundAi`, `_collect`). `doExport` (v1.5.0) additionally offers to embed **`panel.image_svg_trace`** — a worker-traced vector rendition of the raster background (SVG backgrounds are copied in as-is). It is **AI-reading material only**: the briefing tells agents to read it for the drawing's geometry and author their own `image_svg`; `applyImportCore` deletes the field before rendering, so `image_data` always stays the actual background. `doExportBackgroundAi` (v1.3.0) reads the canvas background the same way `embedBackground` does (`#main_image` computed `background-image`), then: raster → confirm offers **vector trace to `.svg`** (v1.4.0, vendored imagetracerjs 1.2.6/public domain inside the userscript; ~1–2 s / ≈7–16 k paths on a Maskin background; small text becomes outlines — inherent to tracing; **v1.5.1: the palette comes from the drawing itself** via `iwdieBuildPalette` → `options.pal` — the tracer's evenly-sampled 16 colours washed flat schematics to grey because thin orange/blue/yellow pipe runs never won a slot against ~99 % white/grey; both trace paths and the CSP main-thread fallback build opts via `traceOptsFor(imgData)`, which MUST run before `traceInWorker` because the worker transfer detaches the ImageData buffer) or the lossless single-page PDF named `.ai` (hand-built, raw-RGB `CompressionStream('deflate')`, artboard = panel px; Illustrator opens any PDF); SVG background → the raw `.svg` (already vector; a PDF wrap would rasterize it). On this plain-http host Chrome may flag downloads — "Keep" saves them.
