...
| Insert uses the `objects_template` append path, which **skips the canvas clear**, and renames from `localStorage["num_of_containers"]` / the live child index | Insert appends. On a populated panel it duplicates every row. Target an empty panel. Container names and `unique_id`s are renumbered by the host, so their absolute values do not survive — only their order and uniqueness matter | §10.1, §12 |
