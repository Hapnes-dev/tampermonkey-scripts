...
6. **Project action buttons** (v1.8.1) — light outline pills left of **Responsible**, plus **🔎 Fetch URLs** left of **Present** (Secondary flex row). Fetch opens a PPT-style chooser with scored Find candidates (signal chips + match %), and **Save URLs** upserts clickable `Attach links:` anchors into the **Internal Quality control and notes** task description. **Delivery to service** auto-fetches IQC / Deal / Delivery URLs on open (Oneflow / Younium step fields; chip lookups only fill gaps).
