...
6. **Project action buttons** (v1.9.0) — light outline pills left of **Responsible**, plus **🔎 Fetch URLs** left of **Present** (Secondary flex row). The **Files** pill opens a PPT-style project-files popover (merged task + folder attachments, preview, download / download-all, upload + drag-drop into General Shared Files). Fetch opens a PPT-style chooser with scored Find candidates (signal chips + match %), and **Save URLs** upserts clickable `Attach links:` anchors into the **Internal Quality control and notes** task description. **Delivery to service** auto-fetches IQC / Deal / Delivery URLs on open (Oneflow / Younium step fields; chip lookups only fill gaps).
