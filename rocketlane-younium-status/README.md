...
7. **Zendesk cases** (v1.10.2) — renames the native **Project updates** tab to **Zendesk cases** (Zendesk-like icon) and replaces that route’s body with the full Project Progress Tracker Zendesk-tasks UI (search merge, last-public-reply hydrate, expand/fullscreen, sanitize, attachments, public/internal reply). Mounts inside `#page` content only so the project top bar stays; first tab click kicks retries. Needs a leading plant number in the project name and a logged-in Zendesk session (same CSRF capture as Delivery).
