...
8. **Home In-progress panel** (v1.12.1) — on `https://kiona.rocketlane.com/` (`pathname === "/"`), PPT-style dark panel of **In progress** projects mounted **under the native Overdue** task section (Incomplete widgets stay). Cards show %, status/due pills, progress bar; owner groups with pin + Due/Progress sort. Data from `POST /projects/lightV1` filtered to Status=In progress (owner/member; exclude `[Tracker] Workload Sync`). `+ RL Project` → `/projects`. **v1.14.2:** the result is cached in Tampermonkey storage, so the panels paint instantly from the last result and refresh in the background once it is older than 5 minutes (status line shows *updated HH:MM* or *refreshing…*); the fetch starts at document-start and pages lightV1 in one parallel round; **Refresh** forces a new fetch. **v1.14.3:** lightV1 is filtered server-side to the projects you own or are a member of (the same `filter` body the Rocketlane Projects grid sends), so one ~1.4 MB page replaces the five 3.7 MB pages of the full scan; the panels also wait for the Overdue card (up to 10 s) instead of mounting at the top and jumping down once it renders.
