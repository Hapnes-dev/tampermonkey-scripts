...
7. **Zendesk cases** (v1.10.3) — renames the native **Project updates** tab to **Zendesk cases** (Zendesk-like icon) and replaces that route’s body with the full Project Progress Tracker Zendesk-tasks UI. Native empty state is hidden via CSS (`body.rlZdCasesActive`); ensure is not run from the MutationObserver (v1.10.2 froze Rocketlane by rewriting styles on every DOM mutation). Needs a leading plant number in the project name and a logged-in Zendesk session.
