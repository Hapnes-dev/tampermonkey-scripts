...
- **Show Details** — fetches the Toolbox SQL API and injects six connection columns (Connection type, Address, Comm port, Baudrate, Parity, Driver addr) straight into the live page grid, then expands every node. See the topology *with* wiring details without leaving the page.

## Install

[Click here to install (latest, currently v1.18)](https://raw.githubusercontent.com/hapnes-dev/tampermonkey-scripts/main/iwmac-topology-copy/IWMAC-Topology-Copy.user.js)
...
Clicking **Show Details** enriches the topology grid *in place* instead of copying or downloading:
